...
func RunGenerate(cmd *cobra.Command, args []string) error {
	if len(args) > 1 {
		...
		if _, err := generate(cmd, args[0], strings.Join(args[1:], " "), nil, wordWrap); err != nil {
			return err
		}
		return nil
	}
...
func generate(cmd *cobra.Command, model, prompt string, history []api.PromptHistory, wordWrap bool) (*api.PromptHistory, error) {
	client, err := api.ClientFromEnvironment()
	if err != nil {
		return nil, err
	}
	...
	request := api.GenerateRequest{Model: model, Prompt: prompt, History: history}
...
func generateInteractive(cmd *cobra.Command, model string) error {
	...
	history := make([]api.PromptHistory, 0)
	...
	for {
		...
		if len(line) > 0 && line[0] != '/' {
			promptReply, err := generate(cmd, model, line, history, wordWrap)
			if err != nil {
				return err
			}
			history = append(history, *promptReply)
		}
	}
}
...
func generateBatch(cmd *cobra.Command, model string) error {
	...
	for scanner.Scan() {
		prompt := scanner.Text()
		fmt.Printf(">>> %s\n", prompt)
		if _, err := generate(cmd, model, prompt, nil, false); err != nil {
			return err
		}
	}
